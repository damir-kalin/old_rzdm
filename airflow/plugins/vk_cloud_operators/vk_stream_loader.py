...
class VKStreamLoader:
    ...
    def execute_stream_load(
        ...
        **properties: Any,
        ...
        """
        # URL encode database and table names for HTTP API
        ...
        url = (
            ...
            f"{encoded_database}/{encoded_table}/_stream_load"
        )
    ...
    def _build_headers(
        ...
        **properties: Any,
        ...
        if columns:
            column_clause = ','.join(
                f"`{col.replace('`', '``')}`" for col in columns
            )
            try:
                column_clause.encode("latin-1")
                headers["columns"] = column_clause
            except UnicodeEncodeError:
                self.log.warning(
                    "Skipping explicit columns list because it contains "
                    "non-Latin-1 characters; relying on table column order."
                )
