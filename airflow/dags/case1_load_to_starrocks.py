...
def get_changed_files(**context):
    ...
    print(f"Обнаружено измененных файлов: {len(changed_files)}")

    # Limit to first 10 files to avoid XCom size limit
    if len(changed_files) > 10:
        print(f"ВНИМАНИЕ: Обнаружено {len(changed_files)} файлов, ограничиваем до первых 10")
...
def get_missing_files(**context):
    ...
    print(f"Обнаружено удаленных файлов: {len(missing_files)}")

    # Limit to first 10 files to avoid XCom size limit
    if len(missing_files) > 10:
        print(f"ВНИМАНИЕ: Обнаружено {len(missing_files)} файлов, ограничиваем до первых 10")
...
def process_changed_file(**context):
    ...
    try:
        ...
        print("=" * 80)
        print(f"НАЧАЛО ОБРАБОТКИ ФИНАНСОВОЙ ФОРМЫ")
        print("=" * 80)
        ...
        print(f"Обработка финансовой формы через FinancialFormsOperator...")
        forms_operator = FinancialFormsOperator(
            ...
            enable_logging=True,  # Enable file status tracking in case_file_metadata
        )
        ...
        if form_result['success']:
            ...
            error_msg = form_result.get('error', 'Неизвестная ошибка')
            print(f"ОШИБКА обработки финансовой формы: {error_msg}")
            raise Exception(f"Ошибка обработки финансовой формы: {error_msg}")
...
def process_missing_file(**context):
    ...
    try:
        print("=" * 80)
        print(f"ОЧИСТКА УДАЛЕННОГО ФАЙЛА")
        print("=" * 80)
        print(f"Файл: {file_name}")
        print(f"Бакет: {bucket_name}")
        ...
        print("=" * 80)
        print(f"КРИТИЧЕСКАЯ ОШИБКА ОЧИСТКИ")
        print("=" * 80)
        print(f"Файл: {file_name}")
        print(f"Ошибка: {str(e)}")
        print(f"Время до ошибки: {processing_time:.2f} сек")
...
with DAG(
    "case1_load_to_starrocks",
    default_args=DEFAULT_ARGS,
    schedule=None,  # Manual trigger for testing
    catchup=False,
    tags=["case1", "starrocks", "excel", "form1", "form2", "financial"],
    max_active_runs=1,
    description="DAG для загрузки Excel Case 1 (Формы 1 и 2) в StarRocks",
) as dag:

    # Sensor for monitoring files in VK Cloud S3 (manual data folder)
    # With PostgreSQL logging enabled for case_file_metadata tracking
    ...
    dbt_run_staging = BashOperator(
        ...
        bash_command=f"""
        cd /opt/airflow/dbt/case_1 && \
        dbt run --profiles-dir . --select models/staging --vars '{{"bucket_name": "{BUCKET_NAME_CLEAN}"}}'
        """,
        trigger_rule=TriggerRule.NONE_FAILED_MIN_ONE_SUCCESS,
    )
    ...
    dbt_run_rdv_hubs = BashOperator(
        ...
        bash_command=f"""
        cd /opt/airflow/dbt/case_1 && \
        dbt run --profiles-dir . --select models/rdv/hubs --vars '{{"bucket_name": "{BUCKET_NAME_CLEAN}"}}'
        """,
    )
    ...
    dbt_run_rdv_links = BashOperator(
        ...
        bash_command=f"""
        cd /opt/airflow/dbt/case_1 && \
        dbt run --profiles-dir . --select models/rdv/links --vars '{{"bucket_name": "{BUCKET_NAME_CLEAN}"}}'
        """,
    )
    ...
    dbt_run_rdv_sats = BashOperator(
        ...
        bash_command=f"""
        cd /opt/airflow/dbt/case_1 && \
        dbt run --profiles-dir . --select models/rdv/sats --vars '{{"bucket_name": "{BUCKET_NAME_CLEAN}"}}'
        """,
    )

    # Stage 5: BDV models
    dbt_run_bdv = BashOperator(
        task_id="dbt_run_bdv",
        bash_command=f"""
        cd /opt/airflow/dbt/case_1 && \
        dbt run --profiles-dir . --select models/bdv --vars '{{"bucket_name": "{BUCKET_NAME_CLEAN}"}}'
        """,
    )
    ...
    dbt_run_staging >> dbt_run_rdv_hubs >> dbt_run_rdv_links >> dbt_run_rdv_sats >> dbt_run_bdv