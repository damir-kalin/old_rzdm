...
def get_changed_files(**context):
    """Получить список измененных файлов для динамической обработки"""
    ...
    print(f"Обнаружено файлов Template 2: {len(template2_files)} из {len(changed_files)} общих")

    # Limit to first 50 files to avoid XCom size limit
    if len(template2_files) > 50:
        print(f"ВНИМАНИЕ: Обнаружено {len(template2_files)} файлов Template 2, ограничиваем до первых 50")
...
def get_missing_files(**context):
    """Получить список удаленных файлов для очистки"""
    missing_files = context['ti'].xcom_pull(
        task_ids='vk_cloud_s3_sensor', key='missing_files'
    )

    if not missing_files:
        print("Нет удаленных файлов для обработки")
        return []

    # Filter to Template 2 files only
    template2_files = [f for f in missing_files if is_template2_file(f['file_name'])]

    print(f"Обнаружено удаленных файлов Template 2: {len(template2_files)} из {len(missing_files)} общих")

    # Limit to first 50 files
    if len(template2_files) > 50:
        print(f"ВНИМАНИЕ: Обнаружено {len(template2_files)} удаленных файлов Template 2, ограничиваем до первых 50")
        return template2_files[:50]
...
def process_changed_file(**context):
    ...
    try:
        print("=" * 80)
        print(f"НАЧАЛО ОБРАБОТКИ TEMPLATE 2 (KPI)")
        print("=" * 80)
        print(f"Файл: {file_name}")
        print(f"Бакет: {bucket_name}")
        print(f"Ключ S3: {key}")
        print(f"Размер: {file_size:,} байт ({file_size / 1024 / 1024:.2f} МБ)")
        print(f"Статус файла: {status}")
        print(f"Схема назначения: {destination_schema}")
        print(f"Таблица StarRocks: {starrocks_table}")
        print(f"S3 Connection: {s3_conn_id}")
        print(f"ID файла: {field}")
        print("=" * 80)

        # Drop existing table if file changed
        if status == "changed":
            print(f"Файл изменен, удаление таблицы {destination_schema}.{starrocks_table}")
            ...
            print(f"✅ Таблица успешно удалена")

        # Process Template 2 file using Template2Operator
        print(f"Обработка Template 2 через Template2Operator...")
        ...
        if result['success']:
            ...
            print(f"✅ Успешно обработано строк: {rows_loaded:,}")
        else:
            error_msg = result.get('error', 'Неизвестная ошибка')
            print(f"ОШИБКА обработки Template 2: {error_msg}")
            raise Exception(f"Ошибка обработки Template 2: {error_msg}")
        ...
        print("=" * 80)
        print(f"ОШИБКА ОБРАБОТКИ TEMPLATE 2")
        print("=" * 80)
        print(f"Файл: {file_name}")
        print(f"Ошибка: {str(e)}")
        print(f"Время до ошибки: {processing_time:.2f} сек")
        print(f"Выполненные шаги: {', '.join(steps_completed) if steps_completed else 'нет'}")
        print("=" * 80)
...
def process_missing_file(**context):
    ...
    try:
        print("=" * 80)
        print(f"ОЧИСТКА УДАЛЕННОГО ФАЙЛА TEMPLATE 2")
        print("=" * 80)
        print(f"Файл: {file_name}")
        print(f"Бакет: {bucket_name}")
        print(f"Схема: {destination_schema}")
        print(f"Таблица: {starrocks_table}")
        print("=" * 80)

        # Drop table in StarRocks
        print(f"Удаление таблицы в StarRocks: {destination_schema}.{starrocks_table}")
        try:
            drop_operator = StarRocksDropTableOperator(
                task_id=f"starrocks_drop_{field}",
                starrocks_database=destination_schema,
            ...
            print(f"Ошибка удаления таблицы: {str(e)}")

        # Drop view in StarRocks
        print(f"Удаление представления в StarRocks: {destination_schema}.v_{starrocks_table}")
        try:
            ...
            if drop_view_result:
                ...
                print(f"✅ Представление успешно удалено")
        except Exception as e:
            steps_completed.append('drop_view_error')
            errors.append(f"drop_view: {str(e)}")
            print(f"Ошибка удаления представления: {str(e)}")
        ...
        print("=" * 80)
        print(f"Время обработки: {processing_time:.2f} сек")
        print(f"Выполненные шаги: {', '.join(steps_completed)}")
        if errors:
            print(f"Предупреждения: {len(errors)}")
        print("=" * 80)
        ...
        print("=" * 80)
        print(f"КРИТИЧЕСКАЯ ОШИБКА ОЧИСТКИ TEMPLATE 2")
        print("=" * 80)
        print(f"Файл: {file_name}")
        print(f"Ошибка: {str(e)}")
        print(f"Время до ошибки: {processing_time:.2f} сек")
...
with DAG(
    "case2_load_to_starrocks",
    default_args=DEFAULT_ARGS,
    schedule=None,  # Manual trigger
    catchup=False,
    tags=["case2", "kpi", "starrocks", "excel"],
    max_active_runs=1,
    description="DAG for loading Case 2 (KPI) Excel files to StarRocks",
) as dag:
    ...
    # Run dbt models in sequential stages after data load
    # Stage 1: Staging models
    dbt_run_staging = BashOperator(
        task_id="dbt_run_staging",
        bash_command="""
        cd /opt/airflow/dbt/case_2 && \
        dbt run --profiles-dir . --select models/staging
        """,
        trigger_rule=TriggerRule.NONE_FAILED_MIN_ONE_SUCCESS,
    )

    # Stage 2: RDV Hub models
    dbt_run_rdv_hubs = BashOperator(
        task_id="dbt_run_rdv_hubs",
        bash_command="""
        cd /opt/airflow/dbt/case_2 && \
        dbt run --profiles-dir . --select models/rdv/hubs
        """,
    )

    # Stage 3: RDV Link models
    dbt_run_rdv_links = BashOperator(
        task_id="dbt_run_rdv_links",
        bash_command="""
        cd /opt/airflow/dbt/case_2 && \
        dbt run --profiles-dir . --select models/rdv/links
        """,
    )

    # Stage 4: RDV Satellite models
    dbt_run_rdv_sats = BashOperator(
        task_id="dbt_run_rdv_sats",
        bash_command="""
        cd /opt/airflow/dbt/case_2 && \
        dbt run --profiles-dir . --select models/rdv/sats
        """,
    )
    ...
    [process_changed_files, process_missing_files] >> summarize_task >> dbt_run_staging
    dbt_run_staging >> dbt_run_rdv_hubs >> dbt_run_rdv_links >> dbt_run_rdv_sats
