...
dag =  DAG('mis_loader_with_parms'
           ...
           , params={
        "topic_name_param": 'sys__mis__esud_rzdm._opt_esud_esud_99099.fdb.H_SCHEDULE'  # Значение по умолчанию, которое будет выводиться в UI при триггере
        , "prefix_table_name_param": 'MIS_'
        , "suffix_table_name_param": ''
        , "offset_begin_param": -1
        , "offset_end_param": -1
        , "ENV_param": 'TEST'
        , "layer_name_param": "main_rdv"
...
def fetch_data_from_kafka(topic_name, **kwargs):
    ...
    '''mis_data = {'payload': {'after': {'ADDRFROMJPERSONS': 0,
                       'AVGROUTETIME': 5,
                       'BNAME': 'Больница 1',
                       'FILIAL': 25111344,
                       'FRMOID': None,
                       'IDBUILD': 990000029,
                       'ISNONUNIQUEADDRESS': None,
                       'JID': 1,
                       'REPL$GRPID': 99,
                       'REPL$ID': 1},
             'before': {'ADDRFROMJPERSONS': 0,
                        'AVGROUTETIME': 5,
                        'BNAME': 'Больница',
                        'FILIAL': 2511,
                        'FRMOID': None,
                        ...
                        'REPL$ID': 1},
             'op': 'UPDATE',
             'source': {'connector': 'reddatabase',
                        'db': '/data/99099.fdb',
                        'name': 'ckb-inf-cpz.rzd-med.ru',
                        'sequence': None,
                        'snapshot': 'false',
                        'table': 'BUILDINGS',
                        'tra_number': 7493011,
                        'ts_ms': 1763033267426500,
                        'version': '${project.version}'},
             'ts_ms': 1763033271781000},
 'schema': {'fields': [{'field': 'op', 'optional': False, 'type': 'string'},
                       ...
                       {'field': 'source',
                        'fields': [{'field': 'version',
                                   ...
                                   {'field': 'table',
                                    'optional': False,
                                    'type': 'string'}],
                        'name': 'io.debezium.connector.firebird.Source',
                        'optional': False,
                        'type': 'struct'},
                       {'field': 'before',
                        'fields': [{'field': 'FILIAL',
                                   ...
                                   {'field': 'IDBUILD',
                                    'optional': False,
                                    'type': 'int64'},
                                   {'field': 'ISNONUNIQUEADDRESS',
                                    'optional': True,
                                    'type': 'int32'},
                                   {'field': 'BNAME',
                                    'optional': True,
                                    'type': 'string'}],
                        'name': 'sys__mis__esud_rzdm._data_99099.fdb.BUILDINGS.Value',
                        'optional': True,
                        'type': 'struct'},
                       {'field': 'after',
                        'fields': [{'field': 'FILIAL',
                        ...
                        'optional': True,
                        'type': 'struct'}],
            'name': 'sys__mis__esud_rzdm._data_99099.fdb.BUILDINGS.Envelope',
            'optional': False,
            'type': 'struct'}}
    kwargs['ti'].xcom_push(key='mis_data', value = mis_data)'''
    print(kwargs['ti'])
...
upload_to_layer = PythonOperator(
    ...
    op_args = ['main_rdv', 'sat_rdv_MIS__', ''],
    dag=dag,
)


fetch_data_from_kafka >> upload_to_storage >> upload_to_layer
