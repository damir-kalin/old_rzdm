{{
  config(
    materialized='table',
    unique_key='link_ind_unit_pk',
    pre_hook="DROP TABLE IF EXISTS {{ this.schema }}.link_ind_unit"
  )
}}
...
hub_unit AS (
  SELECT hub_unit_pk, hub_unit_bk
  FROM {{ ref('hub_unit') }}
),
...
SELECT
  {{ generate_hash(['hi.hub_ind_pk', 'hu.hub_unit_pk']) }} as link_ind_unit_pk,
  hi.hub_ind_pk,
  hu.hub_unit_pk,
...
INNER JOIN hub_unit hu ON s.unit_name = hu.hub_unit_bk
