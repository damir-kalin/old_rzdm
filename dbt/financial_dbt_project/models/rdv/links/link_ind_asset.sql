{{
  config(
    materialized='table',
    unique_key='link_ind_asset_pk',
    pre_hook="DROP TABLE IF EXISTS {{ this.schema }}.link_ind_asset"
  )
}}
...
hub_asset AS (
  SELECT hub_asset_pk, hub_asset_bk
  FROM {{ ref('hub_asset') }}
),
...
SELECT
  {{ generate_hash(['hi.hub_ind_pk', 'ha.hub_asset_pk']) }} as link_ind_asset_pk,
  hi.hub_ind_pk,
  ha.hub_asset_pk,
...
INNER JOIN hub_asset ha ON s.asset_name = ha.hub_asset_bk
