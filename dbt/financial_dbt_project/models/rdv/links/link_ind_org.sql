{{
  config(
    materialized='table',
    unique_key='link_ind_org_pk',
    pre_hook="DROP TABLE IF EXISTS {{ this.schema }}.link_ind_org"
  )
}}
...
SELECT
  {{ generate_hash(['hi.hub_ind_pk', 'ho.hub_org_pk']) }} as link_ind_org_pk,
  hi.hub_ind_pk,
  ho.hub_org_pk,
...
INNER JOIN hub_org ho ON s.org_name = ho.hub_org_bk
