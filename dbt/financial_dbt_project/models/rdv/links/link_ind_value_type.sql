{{
  config(
    ...
    pre_hook="DROP TABLE IF EXISTS {{ this.schema }}.link_ind_value_type"
  )
}}
...
hub_value_type AS (
  SELECT hub_value_type_pk, hub_value_type_bk
  FROM {{ ref('hub_value_type') }}
),
...
SELECT
  {{ generate_hash(['hi.hub_ind_pk', 'hvt.hub_value_type_pk']) }} as link_ind_value_type_pk,
  hi.hub_ind_pk,
...
INNER JOIN hub_value_type hvt ON s.value_type = hvt.hub_value_type_bk
