{{
  config(
    materialized='table',
    unique_key='link_ind_metric_pk',
    pre_hook="DROP TABLE IF EXISTS {{ this.schema }}.link_ind_metric"
  )
}}
...
hub_metric AS (
  SELECT hub_metric_pk, hub_metric_bk
  FROM {{ ref('hub_metric') }}
),
...
SELECT
  {{ generate_hash(['hi.hub_ind_pk', 'hm.hub_metric_pk']) }} as link_ind_metric_pk,
  hi.hub_ind_pk,
  hm.hub_metric_pk,
...
INNER JOIN hub_metric hm ON s.metric = hm.hub_metric_bk
