{{ 
  config(
    alias='link_bdv__ind_economic_activity_type',
    materialized='incremental',
    unique_key='link_ind_economic_activity_type_pk',
    table_type='PRIMARY',
    keys=['link_ind_economic_activity_type_pk'],
    buckets=3,
    enabled=true,
    properties={
      'replication_num': '1'
...
-- Link между показателем и видом экономической деятельности в BDV слое
...
links_with_hubs AS (
  SELECT
    hi.hub_ind_pk,
    h.economic_activity_type_hk as hub_economic_activity_type_pk,
    s.load_dttm,
    s.source_nm
  FROM staging_normalized s
  INNER JOIN {{ ref('hub_bdv__ind') }} hi ON hi.hub_ind_bk = CONCAT_WS('|', 
    s.unit_name_norm,
    s.data_type_norm,
    s.discreteness_norm,
    s.value_type_norm,
  ...
  INNER JOIN {{ source('unverified_rdv', 'hub_rdv__economic_activity_type') }} h ON h.fullname = s.eat_name_norm
),
