...
-- Оптимизированная версия: использует данные напрямую из hub_rdv__fact для уменьшения использования памяти
...
WITH hub_fact_data AS (
  ...
  {% if is_incremental() %}
    WHERE hf.bk_report_date > (SELECT COALESCE(MAX(load_date), '1900-01-01') FROM {{ this }})
  {% endif %}
)
