...
WITH staging_normalized AS (
  SELECT
    ...
    TRIM(REPLACE(s.discreteness, CHAR(160), '')) as date_type_norm,
...
links_with_hubs AS (
  SELECT
    hi.hub_ind_pk,
    h.date_type_hk as hub_date_type_pk,
    s.load_dttm,
    s.source_nm
  ...
  INNER JOIN {{ source('unverified_rdv', 'hub_rdv__date_type') }} h ON h.fullname = s.date_type_norm
),
