...
from {{ ref('hub_bdv__revenue_pmu') }} h_rev
-- подтягиваем рассчитанные показатели
inner join {{ ref('sat_bdv__revenue_pmu') }} s_rev on h_rev.revenue_hk = s_rev.revenue_hk 
-- меняем единицы измерения с рублей на тысячи рублей
inner join {{ ref('sat_bdv__unit_pmu') }} s_unit on s_unit.fullname = 'Тысяча рублей'
