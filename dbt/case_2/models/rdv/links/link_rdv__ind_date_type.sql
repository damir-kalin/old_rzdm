{{
  config(
    ...
    unique_key='link_ind_date_type_pk',
    incremental_strategy='default',
    table_type='PRIMARY',
    keys=['link_ind_date_type_pk'],
    buckets=3,
    properties={
      'replication_num': '1'
    },
...
-- Link между показателем и типом даты (дискретность)
...
WITH staging_with_ind AS (
  ...
  INNER JOIN {{ ref('hub_rdv__ind') }} hi ON 
    ...
    END) = hi.hub_ind_bk
  WHERE s.discreteness IS NOT NULL
    AND s.discreteness != ''
    {% if is_incremental() %}
...
links_with_hubs AS (
  SELECT
    s.hub_ind_pk,
    hdt.date_type_hk as hub_date_type_pk,
    s.load_dttm,
    s.source_nm
  FROM staging_normalized s
  INNER JOIN {{ source('unverified_rdv', 'hub_rdv__date_type') }} hdt ON LOWER(hdt.fullname) = LOWER(s.discreteness_norm)
),
...
link_data AS (
  ...
  GROUP BY hub_ind_pk, hub_date_type_pk
)
