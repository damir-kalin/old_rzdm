...
links_with_hubs AS (
  SELECT
    s.hub_ind_pk,
    hm.metric_hk as hub_metric_pk,
    s.load_dttm,
    s.source_nm
  FROM staging_normalized s
  INNER JOIN {{ source('unverified_rdv', 'hub_rdv__metric') }} hm ON LOWER(hm.fullname) = LOWER(s.metric_norm)
),
...
link_data AS (
  ...
  GROUP BY hub_ind_pk, hub_metric_pk
)
