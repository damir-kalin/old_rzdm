{{
  config(
    ...
    unique_key='link_ind_training_type_pk',
    incremental_strategy='default',
    table_type='PRIMARY',
    keys=['link_ind_training_type_pk'],
    buckets=3,
    properties={
      'replication_num': '1'
    },
...
-- Link между показателем и видом обучения
-- PK формируется как MD5(hub_ind_pk|hub_training_type_pk)
...
staging_normalized AS (
  SELECT
    TRIM(REPLACE(CAST(s.training_type AS CHAR), CHAR(160), '')) as training_type_norm,
    s.hub_ind_pk,
    MAX(s.load_dttm) as load_dttm,
    ANY_VALUE(s.source_nm) as source_nm
  FROM staging_with_ind s
  GROUP BY 
    TRIM(REPLACE(CAST(s.training_type AS CHAR), CHAR(160), '')),
    s.hub_ind_pk
),
...
links_with_hubs AS (
  SELECT
    s.hub_ind_pk,
    htt.training_type_hk as hub_training_type_pk,
    s.load_dttm,
    s.source_nm
  FROM staging_normalized s
  INNER JOIN {{ source('unverified_rdv', 'hub_rdv__training_type') }} htt ON LOWER(htt.fullname) = LOWER(s.training_type_norm)
),

link_data AS (
  SELECT
    MD5(CONCAT_WS('|', CAST(hub_ind_pk AS CHAR), CAST(hub_training_type_pk AS CHAR))) as link_ind_training_type_pk,
    hub_ind_pk,
    hub_training_type_pk,
    MAX(load_dttm) as load_dttm,
    ANY_VALUE(source_nm) as source_nm
  FROM links_with_hubs
  GROUP BY hub_ind_pk, hub_training_type_pk
)

SELECT
  link_ind_training_type_pk,
  hub_ind_pk,
  hub_training_type_pk,
  load_dttm,
  source_nm
