...
links_with_hubs AS (
  SELECT
    s.hub_ind_pk,
    hu.unit_hk as hub_unit_pk,
    s.load_dttm,
    s.source_nm
  FROM staging_normalized s
  INNER JOIN {{ source('unverified_rdv', 'hub_rdv__unit') }} hu ON LOWER(hu.fullname) = LOWER(s.unit_name_norm)
),
...
link_data AS (
  ...
  GROUP BY hub_ind_pk, hub_unit_pk
)
