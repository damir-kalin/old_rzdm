-- ============================================
-- Case file metadata tables and functions
-- ============================================

-- Создание таблицы case_file_metadata
CREATE TABLE IF NOT EXISTS public.case_file_metadata (
    file_id TEXT,
    file_name TEXT,
...
CREATE TABLE IF NOT EXISTS public.case_file_metadata_history (
    file_id TEXT,
    file_name TEXT,
    user_creator_name TEXT,
    last_modified TIMESTAMP NOT NULL,
    ...
    updated_at TIMESTAMP DEFAULT NOW(),
    key VARCHAR(255),
    CONSTRAINT fk_case_hist_file_bucket
        FOREIGN KEY (bucket_id)
        REFERENCES buckets(bucket_id)
        ON DELETE RESTRICT
        ON UPDATE CASCADE,
    CONSTRAINT fk_case_hist_file_status
        FOREIGN KEY (file_load_status_id)
        REFERENCES processing_status(status_id)
        ON DELETE RESTRICT
        ON UPDATE CASCADE,
    CONSTRAINT fk_case_hist_file_error
        FOREIGN KEY (error_id)
        REFERENCES processing_error(error_id)
        ON DELETE SET NULL
        ON UPDATE cascade
...
CREATE OR REPLACE FUNCTION log_case_file_metadata_change()
RETURNS TRIGGER AS $$
BEGIN
    INSERT INTO public.case_file_metadata_history (
        file_id,
        file_name,
        user_creator_name,
        last_modified,
...
CREATE INDEX IF NOT EXISTS idx_case_file_metadata_key ON public.case_file_metadata(key);




-- Функция возвращает записи из case_file_metadata, которых нет в текущем списке ключей S3
CREATE OR REPLACE FUNCTION public.get_missing_s3_case_files(current_keys text[])
RETURNS TABLE (
    key text,
    file_name text,
    user_name text,
...
CREATE TRIGGER trg_case_file_metadata_history
AFTER INSERT OR UPDATE ON public.case_file_metadata
FOR EACH ROW
EXECUTE FUNCTION log_case_file_metadata_change();




 CREATE OR REPLACE FUNCTION check_update_case_file_s3(
...
IF NOT EXISTS (SELECT 1 FROM public.case_file_metadata as fm inner join public.buckets as b on fm.bucket_id = b.bucket_id
                            WHERE fm.file_name = p_file_name
                                and b.bucket_name = p_bucket
                    ) THEN

        INSERT INTO public.case_file_metadata (
            file_id,
            file_name,
            user_creator_name,
            last_modified,
        ...
        RETURN 1;
    ELSIF EXISTS (SELECT 1 FROM public.case_file_metadata as fm inner join public.buckets as b on fm.bucket_id = b.bucket_id
                            WHERE fm.file_name = p_file_name
                                and b.bucket_name = p_bucket
                                and (fm.last_modified != p_last_modified or fm.size != p_size)
                                and not exists (select 1 from public.case_file_metadata as f where f.key=p_key)) THEN
...
CREATE OR REPLACE FUNCTION update_status_case_file_s3(
    p_guid VARCHAR(255),
    p_status_code VARCHAR(255)
)
RETURNS bool AS $$
BEGIN
    IF EXISTS(SELECT 1 FROM public.case_file_metadata as fm where fm.file_id=p_guid) THEN
  IF p_status_code='extension_error' THEN
    UPDATE public.case_file_metadata
          SET error_id=1,
